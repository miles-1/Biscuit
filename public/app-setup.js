...
const WORKFLOW_STEP_TOOLTIPS = {
    ...
    'feedback-pdfs': 'Step [3] generates these per-student feedback `.pdf`s exported after grading.\n - #drive If the class roster included `student_email` when the assignment was created (see step [1]), Finish & Export can upload PDFs under `Biscuit/class_name` in Google Drive after a one-time Google account link.',
    'grades-csvs': 'Step [3] exports two scores spreadsheets: one detailed version with per-question information, and a second with student totals.\n - #canvas If the class roster included `student_id` when the assignment was created (see step [1]), the second spreadsheet can be uploaded directly to Canvas for score submission.',
