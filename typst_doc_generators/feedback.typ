  ...
  for (assn_id_str, assn_data) in grading_data {
    ...
    for q in questions {
      ...
      if has_feedback or has_manual_answer {
        let notes = if has_manual_answer [_Note: the computer detected that you answered_ #format_answer(q.answer). _After review, the grader determined that this was incorrect, and changed your answer to be marked as_ #format_answer(q.manual_answer, backup:q.answer). #if has_feedback {linebreak()}] + if has_feedback {eval(q.feedback, mode:"markup")}
        question_notes.push([
